...
async def get_setting(key: str) -> Optional[str]:
    ...
    mapping = {k: v for k, v in zip(keys[1:], values[1:])}  # skip header
...
async def get_watchlist() -> List[str]:
    ...
    if not header:
        ...
        return []
    assets = ws.col_values(1)[1:]  # skip header
...
@cached(ttl=120)
async def get_mutual_fund_row(name_or_code: str) -> Optional[Dict[str, Any]]:
    ws = _worksheet("mutual_funds")
    header = ws.row_values(1)
    if not header:
        ws.update("A1:D1", [["Name", "Code", "NAV_IDR", "Date"]])
        return None
    rows = ws.get_all_records()
    q = name_or_code.strip().lower()
    for r in rows:
        if str(r.get("Name", "")).lower() == q or str(r.get("Code", "")).lower() == q:
            return r
    # partial match
    for r in rows:
        if q in str(r.get("Name", "")).lower() or q in str(r.get("Code", "")).lower():
            return r
    return None

async def diag_info():
    # Return diagnostic info including permissions to critical tabs
    ok = True
    details = {}
    try:
    ...
    try:
        _ = _worksheet("mutual_funds")
        details["mutual_funds"] = "ok"
    except Exception as e:
        ok = False
        details["mutual_funds"] = f"error: {e}"
